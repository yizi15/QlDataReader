...
def modify_bytes(data, offset, new_data):
    return data[:offset] + new_data + data[offset + len(new_data):]


def repir_header(conver_name, right_bin_user_info, right_bin_data_info):
    qle_data_size = os.path.getsize(conver_name)
    base_name = os.path.basename(conver_name)
    header_data_user = x7qledef.STHeader.unpack(right_bin_user_info[:85])
    extra_data_user = x7qledef.T_RecoderFileExtraData.unpack(right_bin_user_info[85:])
    header_data_data = x7qledef.STHeader.unpack(right_bin_data_info[:85])
    extra_data_data = x7qledef.T_RecoderFileExtraData.unpack(right_bin_data_info[85:])
    
    header_data_user['magic'] = header_data_data['magic']
    header_data_user['channel'] = header_data_data['channel']
    header_data_user['sampleRate'] = header_data_data['sampleRate']
    header_data_user['points_pre_package'] = header_data_data['points_pre_package']
    header_data_user['bype_per_point'] =   header_data_data['bype_per_point']
    
    extra_data_user['prefix_data_offset'] = extra_data_data['prefix_data_offset']

    extra_data_user['phy_max'] = extra_data_data['phy_max']
    extra_data_user['phy_min'] = extra_data_data['phy_min']
    extra_data_user['digital_max'] = extra_data_data['digital_max']
    extra_data_user['digital_min'] = extra_data_data['digital_min']
    extra_data_user['phy_unit'] = extra_data_data['phy_unit']

    header_data_ori = x7qledef.STHeader.pack(header_data_user)
    extra_data_ori = x7qledef.T_RecoderFileExtraData.pack(extra_data_user)
    return header_data_ori, extra_data_ori
...
if __name__ == '__main__':
    def read_534(name):
        with open(name, 'rb') as f:
            return f.read(534)
    in_name = r'C:\Users\fengxinan\Downloads\177345\177345_acc.acc'
    right_header = repir_header(in_name, read_534(in_name[:-7] + 'eeg.eeg'), read_534(r'C:\Users\fengxinan\Downloads\177350\177350_acc.acc'))
    with open(in_name, 'rb') as f:
        f.seek(534)
        right_data = bytes(right_header[0]) + bytes(right_header[1]) + f.read()
